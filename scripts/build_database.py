...
from db import Row, connect as connect_database, database_url
...
def configure_database_connection(conn: Any) -> None:
    return None
...
def create_schema(conn: Any) -> None:
    conn.executescript(
        """
        DROP TABLE IF EXISTS external_metadata CASCADE;
        DROP TABLE IF EXISTS album_service_status CASCADE;
        DROP TABLE IF EXISTS cover_art CASCADE;
        DROP TABLE IF EXISTS album_genres CASCADE;
        DROP TABLE IF EXISTS tracks CASCADE;
        DROP TABLE IF EXISTS musicbrainz_metadata CASCADE;
...
def ensure_track_preview_schema(conn: Any) -> None:
    track_columns = {
        row["column_name"]
...
def import_catalog(conn: Any, rows: list[dict[str, str]]) -> None:
    insert_sql = """
        INSERT INTO albums (
...
def sanitize_cached_urls(conn: Any) -> None:
    rows = conn.execute(
        """
        SELECT provider, cache_key, url
        FROM api_cache
        WHERE provider = 'discogs' AND url LIKE ?
        """,
        ("%token=%",),
    ).fetchall()
...
def upsert_metadata(conn: Any, album_id: int, metadata: dict) -> None:
    columns = [
        "album_id",
        "lookup_status",
        "lookup_error",
...
def replace_tracks(conn: Any, album_id: int, release: dict | None) -> int:
...
def replace_musicbrainz_genres(conn: Any, album_id: int, release: dict | None) -> int:
    ...
    conn.executemany(
        """
        INSERT INTO album_genres (album_id, source, name, count)
        VALUES (?, ?, ?, ?)
...
def musicbrainz_genre_names(conn: Any, album_id: int) -> list[str]:
    rows = conn.execute(
        """
        SELECT name
        FROM album_genres
...
def upsert_musicbrainz_external_metadata(conn: Any, album_id: int, metadata: dict) -> None:
...
def lastfm_master_fields(row: Row | None) -> dict[str, str | None]:
    if not row or row["lookup_status"] != "matched":
        return {}
    return {"genre": unique_join(json_list(row["genres"]), limit=6), "compilation": is_various_artist(row["artist"])}


def apple_master_fields(row: Row | None) -> dict[str, str | None]:
    if not row or row["lookup_status"] != "matched":
        return {}
    raw = json.loads(row["raw_json"] or "{}")
...
def musicbrainz_master_fields(conn: Any, album_id: int) -> dict[str, str | None]:
...
def update_master_catalog_fields(conn: Any, album_id: int) -> None:
    external_rows = {
        row["provider"]: row
        for row in conn.execute(
...
def fetch_cover_art(conn: Any, album_id: int, mb_release_id: str | None, refresh_cache: bool) -> int:
...
def upsert_artist(conn: Any, metadata: dict) -> None:
    columns = [
        "name",
        "lookup_status",
        "lookup_error",
...
def fetch_lastfm_artist(conn: Any, artist: str, refresh_cache: bool) -> None:
...
def insert_discogs_tracks(conn: Any, album_id: int, payload: dict, replace_existing: bool = False) -> int:
...
def replace_tracks_from_cached_discogs_match(conn: Any, album_id: int) -> int:
    row = conn.execute(
        """
        SELECT raw_json
        FROM external_metadata
...
def update_apple_track_previews(conn: Any, album_id: int, apple_tracks: list[dict]) -> int:
    existing = conn.execute(
...
def fetch_apple_collection_id(conn: Any, album_id: int, collection_id: str, refresh_cache: bool) -> tuple[str | None, str | None]:
    lookup_params = urllib.parse.urlencode({"id": collection_id, "entity": "song", "country": "US"})
    lookup_url = f"https://itunes.apple.com/lookup?{lookup_params}"
...
def enrich_musicbrainz_release_id(conn: Any, album_id: int, release_id: str, refresh_cache: bool) -> tuple[str | None, str | None]:
...
def enrich_musicbrainz_by_search(conn: Any, album_id: int, artist: str, album_name: str, refresh_cache: bool) -> tuple[str | None, str | None]:
...
def fetch_lastfm_album_info(conn: Any, album_id: int, artist: str, album_name: str, refresh_cache: bool) -> tuple[str | None, str | None]:
    api_key = os.environ.get("LASTFM_API_KEY", "").strip()
    if not api_key:
...
def enrich_album_from_service_url(conn: Any, album_id: int, service_url: str, refresh_cache: bool = False) -> dict:
    load_dotenv()
...
def enrich_album_rows(conn: Any, rows: list[Row], refresh_cache: bool) -> None:
    for row in rows:
        album_id = row["id"]
...
def enrich_first_albums(conn: Any, limit: int, offset: int, refresh_cache: bool) -> None:
    rows = conn.execute(
        """
...
def refresh_catalog_ids(conn: Any, catalog_ids: list[str]) -> None:
    if not catalog_ids:
        return
...
def fetch_apple_preview_tracks(conn: Any, artist: str, album_name: str, refresh_cache: bool) -> tuple[list[dict], str | None]:
    query = f"{artist} {album_name}".strip()
    params = urllib.parse.urlencode({"term": query, "media": "music", "entity": "album", "limit": "25", "country": "US"})
    url = f"https://itunes.apple.com/search?{params}"
    payload, _, error = cached_json(conn, "apple_itunes", f"album-search:{query}", url, refresh_cache=refresh_cache)
...
def scan_itunes_preview_links(conn: Any, limit: int, offset: int, refresh_cache: bool) -> None:
    rows = conn.execute(
        """
...
def main() -> None:
    ...
    parser.add_argument("--database-url", default=DATABASE_URL, help="Postgres connection URL. Defaults to DATABASE_URL or local Docker defaults.")
    parser.add_argument("--db", type=Path, help="Deprecated; use DATABASE_URL or --database-url.")
    ...
    if args.db:
        print("--db is deprecated for Postgres and will be ignored. Use DATABASE_URL or --database-url.")
    ...
    if db_exists:
        print("Updated existing Postgres database.")
    else:
        print(f"Wrote {imported_rows} catalog rows to Postgres.")
