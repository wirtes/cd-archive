...
def open_preview_database():
    conn = connect_database(DATABASE_URL)
    schema_name = f"preview_{secrets.token_hex(8)}"
...
class CatalogHandler(SimpleHTTPRequestHandler):
    ...
    def handle_create_scan_event(self):
        ...
        with self.db() as conn:
            row = conn.execute(
                "INSERT INTO scan_events (username, created_at, payload_json) VALUES (?, ?, ?) RETURNING id",
            ...
            where.append(
                ...
                (
                    ...
                    OR EXISTS (
                        ...
                        WHERE external_metadata.album_id = albums.id
                          AND LOWER(TRIM(genre.value)) = LOWER(TRIM(?))
                    )
                    OR EXISTS (
                        SELECT 1
                        FROM external_metadata
                        ...
                        WHERE external_metadata.album_id = albums.id
                          AND LOWER(TRIM(style.value)) = LOWER(TRIM(?))
                    )
                )
                """
            )
        ...
        with self.db() as conn:
            ...
            rows = conn.execute(
                ...
                SELECT
                    ...
                    (
                        SELECT string_agg(provider, ', ' ORDER BY sort_order, provider)
                        FROM (
                            SELECT provider,
                                   CASE provider
                                       WHEN 'apple_itunes' THEN 1
    ...
    def insert_album(self, conn, form, row_number=None):
        ...
        row = conn.execute(
            f"INSERT INTO albums ({', '.join(ALBUM_INSERT_COLUMNS)}) VALUES ({placeholders}) RETURNING id",
    ...
    def handle_music_service_preview(self):
        ...
        schema_name = None
        try:
            conn, schema_name = open_preview_database()
...
def main():
    with connect_database(DATABASE_URL) as conn:
        if not database_has_schema(conn):
            raise SystemExit("Database schema does not exist yet. Run scripts/build_database.py first.")
